<template>
  <div class="w-full">
    <div class="text-center mb-8">
      <h2 class="text-2xl font-headline font-bold text-secondary">Registro de Estudiante</h2>
    ...
    <form @submit.prevent="handleNextStep" class="space-y-4">
      ...
      <div v-if="step === 1" class="space-y-4 animate-fade-in-up">
        ...
        <div>
          <label class="block text-xs font-bold text-secondary mb-1">Correo Institucional</label>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-xs font-bold text-secondary mb-1">Registro Universitario (RU)</label>
            <input type="text" class="w-full bg-neutral text-sm rounded-lg py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-primary/50 border border-transparent focus:border-primary" required />
          </div>
          <div>
            <label class="block text-xs font-bold text-secondary mb-1">C.I.</label>
            <input type="text" class="w-full bg-neutral text-sm rounded-lg py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-primary/50 border border-transparent focus:border-primary" required />
          </div>
        </div>

        <div>
          <label class="block text-xs font-bold text-secondary mb-1">Carrera</label>
          <select class="w-full bg-neutral text-sm rounded-lg py-2.5 px-3 focus:outline-none focus:ring-2 focus:ring-primary/50 border border-transparent focus:border-primary" required>
      ...
      <div v-if="step === 2" class="space-y-4 animate-fade-in-up">
        <div>
          ...
          <p class="text-[10px] text-gray-500 mt-1">Usa al menos 8 caracteres con números y símbolos.</p>
        </div>
        ...
        <div class="flex items-start gap-2 mt-6">
          ...
          <label for="terminos" class="text-xs text-gray-500 leading-tight">
            He leído y acepto los <a href="#" class="text-primary hover:underline font-bold">Términos de Servicio</a> y la <a href="#" class="text-primary hover:underline font-bold">Política de Privacidad</a> de la UMSA.
          </label>
        </div>
      </div>
...
const handleNextStep = () => {
  if (step.value === 1) {
    ...
    // Pasar a confirmación enviando el rol como query param para mostrar el check correcto
    router.push({ path: '/auth/confirmacion', query: { rol: 'estudiante' } })
  }
}
</script>
